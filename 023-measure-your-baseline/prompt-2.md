# Baseline Prompt — Re-Run Against a Bloated Copy

**Article:** 023 — Measure your baseline (Prompt 2 of 2; Prompt 1 is `prompts/baseline.md`).
**Surfaces:** Claude Code primary; the same logic adapts to ChatGPT custom instructions and OpenAI/Anthropic API system prompts.
...
```bash
rm -rf "$HOME/.claude/experiments/$(date +%Y-%m-%d)-023-bloat-delta"
```

That is the entire revert. The prompt copies `~/CLAUDE.md` into scratch but never writes to the original. Deleting the scratch directory restores you to your starting state.

## What you should now see

- A clean clone of your `~/CLAUDE.md` and a bloated clone differing by exactly the paragraph above.
- Two `00-BASELINE.md` files differing in exactly `memory_files_total_tokens`.
- A delta of roughly 540 tokens — the cost of the inserted paragraph at the words-per-token ratio established in Article 021.
- A B0 flag if you do not yet have `~/CLAUDE.md`, pointing toward Article 037.
- A working comparison ritual you will use for the rest of the series: snapshot, change, re-snapshot, diff.

## How to confirm the win

Article 023 is value-tagged `both` and *establishes* the baseline. The proof of correctness is small and direct: open `$SCRATCH/00-DELTA.md` and confirm that exactly one number changed between the clean and bloated runs, that the change is in `memory_files_total_tokens`, and that the magnitude (≈540) is consistent with the inserted paragraph's word count divided by 0.75. If any other field changed, the baseline is reading state that is not file-content-derived and should be investigated before trusting it as a comparison ritual. If the magnitude is wildly off (>30% from expected), recheck the token-estimation method.

Subscribe so you don't miss Article 024, where the inventory question becomes "which model do I actually need for this turn?"
